...
export function FinanceHeader({ onMenuClick }: HeaderProps) {
                ...
                <DropdownMenuTrigger asChild>
                    <button className="flex items-center gap-3 pl-1 pr-2 py-1 rounded-full hover:bg-slate-100 transition-colors focus:outline-none ring-offset-2 focus:ring-2 focus:ring-indigo-500/20">
                        ...
                        <div className="hidden md:flex flex-col items-start text-sm">
                            <span className="font-semibold leading-none text-slate-700">{user?.fullName || "Staff Keuangan"}</span>
                            <span className="text-xs text-slate-500 mt-0.5">{user?.role || "Finance"}</span>
                        </div>
                ...
                <DropdownMenuContent align="end" className="w-56 p-2">
                    <DropdownMenuLabel className="font-normal">
                        <div className="flex flex-col space-y-1">
                            ...
                            <p className="text-xs leading-none text-muted-foreground">finance@enterprise.com</p>
                        </div>
                    </DropdownMenuLabel>
                    <DropdownMenuSeparator />
